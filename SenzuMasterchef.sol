...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    SenzuToken public senzu;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public senzuPerBlock;
    // Bonus muliplier for early senzu makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _senzuPerBlock,
        uint256 _startBlock
    ) public {
        senzu = _senzu;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        senzuPerBlock = _senzuPerBlock;
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 700, "set: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 senzuReward = multiplier.mul(senzuPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(senzuReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 senzuReward = multiplier.mul(senzuPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        senzu.mint(devaddr, senzuReward.div(10));
        senzu.mint(address(this), senzuReward);
        pool.accEggPerShare = pool.accEggPerShare.add(senzuReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe senzu transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 senzuBal = senzu.balanceOf(address(this));
        if (_amount > senzuBal) {
            senzu.transfer(_to, senzuBal);
        } else {
            senzu.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _senzuPerBlock) public onlyOwner {
        massUpdatePools();
        senzuPerBlock = _senzuPerBlock;
    }
}